...
function LeafletMap(containerId) {
	...
	wax.tilejson('http://a.tiles.mapbox.com/v3/mapbox.world-blank-light.jsonp', function(tilejson) {
...
LeafletMap.prototype = {
	...
	addPolygon: function(geo) {
		...
		if (geo.id.toLowerCase() !== 'rs') {
			// Decode polygon data, replacing encoded data with decoded data.
			for (i=geo.shape.length-1; i >= 0; i--) {
		        geo.shape[i] = pts = google.maps.geometry.encoding.decodePath(geo.shape[i]);

				// Transcode coords into Leaflet LatLng objects.
				for (j in pts) {
					if (pts.hasOwnProperty(j)) {
						pt = pts[j];
						pts[j] = new L.LatLng(pt.lat(), pt.lng());
					}
				}
		    }
		} else {
			// Special data/encode for Russia... going through Google encoding messes up point wrapping.
			if (!(rs[0][0][0] instanceof L.LatLng)) {
				for (i in rs) {
		...
		geo.shape = new L.MultiPolygon(geo.shape, {
			stroke: false,
			fill: true,
			fillColor: this.getColorForValue( geo.value ),
			fillOpacity: 0.9,
		...
		if (this.tooltip) {
			...
			geo.shape.id = geo.id;
		    geo.shape.on('mouseover', this.onMouseOver, this);
		    geo.shape.on('mouseout', this.onMouseOut, this);
			geo.shape.on('click', this.onClick, this);
		}

		// Attach polygon to map.
		this.map.addLayer(geo.shape);
	},
	...
	clearMap: function() {
		...
		if (this.map && this.mapData) {
			while (this.mapData.length > 0) {
				...
				geo.shape.off('mouseover', this.onMouseOver, this);
			    geo.shape.off('mouseout', this.onMouseOut, this);
				geo.shape.off('click', this.onClick, this);
				this.map.removeLayer(geo.shape);
				delete this.mapDataById[ geo.id ];
				geo.shape = null;
				geo = null;
			}
			this.mapData = null;
			this.mapDataById = null;
		}
	},
